...
for k=1:numepochs
    ...
    disp(['NN train:epoch ' num2str(k) '/' num2str(opts.numepochs) '.Took' num2str(t) 'second' '.Mini-batch mean squared error on training set is ' num2str(mean(L((n-numbatches):(n-1)))) str_perf]);
end
end
...
%error and loss
nn.error=y-nn.a{n};
nn.loss=0.5*sum(sum(nn.error.^2))/m;
end
function nn=nnbp(nn)
n=nn.n;
d{n}=-nn.error .*(nn.a{n}.*(1-nn.a{n}));
for i=(n-1):-1:2
    % Derivative(ÅÉÉú) of the activation function
    d_act=nn.a{i}.*(1-nn.a{i});
    % Backpropagate first derivatives
    if i+1==n % in this case in d{n} there is not the bias term to be removed
        d{i}=(d{i+1}*nn.W{i}).*d_act;
    else % in this case in d{n} the bias term has to be rempved
